...
.globl sys_map_is_solid_at    ;; B=pixel_y, C=pixel_x_bytes → NZ if fully solid (ceiling/sides)
.globl sys_map_is_landable_at ;; B=pixel_y, C=pixel_x_bytes → NZ if solid or jumpable (floor)
