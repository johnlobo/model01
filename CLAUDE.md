...
The same command validates compile-time map/physics ranges and fails if the
linked binary crosses `0x7FFF`, the end of the live banking window.
...
`game_menu_version` in `src/game/menu.s` (e.g. `"VERSION - V.083"`) is displayed at the bottom of the main menu. **Bump this after every significant change.**
